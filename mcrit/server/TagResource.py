...
from mcrit.libs.tags import TAG_ENTITIES, normalizeTags
...
class TagResource:
    ...
    synchronous, answer 200 with the entity's resulting tags, 400 for a malformed body or an
    invalid tag - without touching the entity - and 404 for an unknown id. There is deliberately
    no replace-all: two analysts tagging the same entity at once cannot erase each other's tags.
    The instance without an entity serves ``GET /tags?entity=family|sample|function``, the
    distinct tags of that entity with the number of entities carrying each.
    """
    ...
    def _change_tags(self, req, resp, route_params, is_adding):
        ...
        try:
            tags = normalizeTags(req.media["tags"])
        ...
        db_log_msg(self.index, req, f"TagResource.{responder} - success - {self.entity} {entity_id} {'+' if is_adding else '-'}{tags}.")
